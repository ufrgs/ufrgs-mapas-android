 ...
 */
/**
 * Persistent search library override. Required to show results that don't start with the search query.
 */
package br.ufrgs.ufrgsmapas.libs;
...
public class SearchBox extends RelativeLayout {
	...
	// Maximum number of result on screen.
	public static final int mResultSizeLimit = 10;
	...
	public void updateResults() {
		...
		for (int x = 0; x < searchables.size(); x++) {
			if (searchables.get(x).title.toLowerCase().contains( ////////////////////////////////////////////////////////////
					getSearchText().toLowerCase())
					&& count < mResultSizeLimit) {
	...
	private void addResult(SearchResult result) {
		if (resultList != null && resultList.size() < mResultSizeLimit) {
